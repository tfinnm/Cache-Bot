...
public class stats extends command {
	...
	public void action(Message msg) {
		...
		cmds.add(new Field("Servers", String.valueOf(launcher.jda.getGuilds().size()), false));
		int totalMembers = 0;
		int totalChannels = 0;
		int totalBoosts = 0;
		...
		cmds.add(new Field("Synced Bans", String.valueOf(Data.blacklist.size()), true));
		cmds.add(new Field("Active Advisories", String.valueOf(Data.advisories.size()), true));
		cmds.add(new Field("Ping", String.valueOf(launcher.jda.getGatewayPing()), true));
		cmds.add(new Field("Shard", launcher.jda.getShardInfo().getShardString(), true));
		cmds.add(new Field("Uptime Events", String.valueOf(launcher.jda.getResponseTotal()),true));
